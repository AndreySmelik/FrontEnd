...
export class TeamTableDataSource extends DataSource<TeamTableItem> {
  getTeam() {
    ...
    handleTeam(res:any){
       if (res){
        console.log('call res', res)
         res.forEach(element => {
          let dataTableItem: TeamTableItem = {id: 0, name: '',region:''};
          dataTableItem.id = element.id;
          dataTableItem.name = element.teamName;
          dataTableItem.region = element.region;
    ...
    data: TeamTableItem[]=[];
  constructor(private paginator: MatPaginator, private sort: MatSort,private teamService: TeamService) {
    super();
    this.getTeam();
  }
